      ...
      <div id="container">
        <ion-card v-for="panel in panels" :key="panel.id" class="custom-rounded">
          <ion-card-content>
            ...
            <ion-button @click="editPanel(panel.id)">
              <ion-icon name="create-outline" size="large"></ion-icon>
            </ion-button>
            <ion-button @click="deletePanel(panel.id)">
...
export default defineComponent({
  ...
  methods: {
      ...
      if (panel) {
        ...
        axios.put(`http://localhost:9000/Tasky/api/Panel/tareas/${panelId}`, { evento: 1 })
          .then(response => {
            ...
            this.panels = this.panels.filter(p => p.id !== panelId); // Remover panel del listado local
          })
          .catch(error => {
      ...
      axios.get(`http://localhost:9000/Tasky/api/Usuario?email=${emailGuardado}`)
        .then(response => {
          if (response.data.length > 0) {
            const usuario = response.data[0];
            const idUsuario = usuario.id;
            axios.post(`http://localhost:9000/Tasky/api/Panel`, { ...this.panelForm, userId: idUsuario })
              .then(() => {
                this.mostrarPaneles(idUsuario);
    ...
    updatePanel() {
      axios.put(`http://localhost:9000/Tasky/api/Panel/${this.panelForm.id}`, this.panelForm)
        .then(() => {
    ...
    deletePanel(panelId) {
      axios.delete(`http://localhost:9000/Tasky/api/Panel/${panelId}`)
        .then(() => {
